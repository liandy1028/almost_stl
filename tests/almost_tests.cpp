...
DUCK_TEST(Pushback, Lvalue) {
  T<> a;
  Vector<T<>> v(2);
  v.push_back(a);
  v.push_back(a); // no reallocation
}

DUCK_TEST(Insert, Lvalue) {
  T<> a;
  Vector<T<>> v(2);
  v.insert(v.begin() + 1, a);
  v.insert(v.begin() + 1, a); // no reallocation
}

DUCK_TEST(Insert, LvalueAtEnd) {
  T<> a;
  Vector<T<>> v(2);
  v.insert(v.end(), a);
  v.insert(v.end(), a); // no reallocation
}

DUCK_TEST(Insert, LvalueCount) {
  T<> a;
  Vector<T<>> v(2);
  v.insert(v.begin() + 1, 1, a);
  v.insert(v.begin() + 1, 1, a); // no reallocation
}

DUCK_TEST(Insert, LvalueCountAtEnd) {
  T<> a;
  Vector<T<>> v(2);
  v.insert(v.end(), 1, a);
  v.insert(v.end(), 1, a); // no reallocation
}

DUCK_TEST(Emplace, Lvalue) {
  T<> a;
  Vector<T<>> v(2);
  v.emplace(v.begin() + 1, a);
  v.emplace(v.begin() + 1, a); // no reallocation
}

DUCK_TEST(Emplace, LvalueAtEnd) {
  T<> a;
  Vector<T<>> v(2);
  v.emplace(v.end(), a);
  v.emplace(v.end(), a); // no reallocation
}

DUCK_TEST(Emplace, LvalueWithinContainer) {
  Vector<T<>> v{1, 2};
  v.emplace(v.begin() + 1, v[0]);
  LOG(v, "\n");
  v.emplace(v.begin() + 1, v[0]); // no reallocation
  LOG(v, "\n");
}

DUCK_TEST(Resize, CopyIsCreated) {
  T<> a;
  Vector<T<>> v;
  v.reserve(1);
  v.resize(1, a);
